...
int main() {
    ...
    {
        veci[i].erase(veci[i].begin(), veci[i].end()-5);
        veci[i].erase(std::remove(veci[i].begin(), veci[i].end(), ':'), veci[i].end());
        
        stringstream intValue(veci[i]);
        intValue >> veci[i];
        cout << veci[i]<< endl;
        auto s = to_string(i);
        HT.insertItem(vec[i], s);
    }
    cout << "jola" << endl;
    ...
    cout << "inserte la ip sin puntos" << endl;
    int key;
    cin >> key;
    cout << "esta ip se encuentra en las posiciones" << endl;
